...
- `this`对象在JS中指向方法所属的对象，不在方法中时（或者方法是全局方法时），就指向全局对象（在浏览器中是window）。方法可以通过`call()`或者`apply()`方法向A对象的方法注入B对象的实例，来达到切换上下文的目的。
...
- ES6中的**解构操作**，当解构结果为忽略时，其表示为空格，当解构结果为剩余值时，其表示为`...`，解构的默认值为`undefined`，设置默认值时和对方法设置默认值一致:
