...
1. 监听 Activity（通过 Application）、Fragment（通过Activity 的 FragmentManager）、ViewModel（通过反射获取 ViewModelStore 的 map 字段，手动 put 一个监听用 ViewModel 进去） 等 onDestory 回调，用ObjectWatcher 持有它们的软引用。等5秒并 GC 后，如果对应的对象仍然存在，则认为它是滞留状态，如果滞留对象数超过其阈值（默认是5），就dump the heap。
2. 未完待续
